...
void BFreq_Pst_S_user_init(test_function& func)
{
	BFreq_Pst_S_params *ours;
    ours = (BFreq_Pst_S_params *)func.params;
...
// Datalog gFuncNum variable //
	if(gDisplay_FuncNum)
		PiDatalog(func, A_Func_Num_BFreq_Pst_S, gFuncNum, 27, POWER_UNIT);
	...
	int   i             = 0;
	float BFreq_Pst_S   = 0;
	float BFreq_Delta_S = 0;
	float BFreq_Ton_S   = 0;
	float BFreq_DC_S    = 0;
	float BFreq_Test1_S = 0;
	float BFreq_Test2_S = 0;
	float BFreq_Pst_TT  = 0;
	float temp_1        = 0;
	float period        = 0;
	float tmeas         = 0;
	int   num_loop      = 0;
...
Pulse pulse;
	...
	tmu_6->start_trigger_setup(3.0, NEG_SLOPE, TMU_HIZ, TMU_IN_10V);
	tmu_6->stop_trigger_setup(3.5, POS_SLOPE, TMU_HIZ, TMU_IN_10V);
	tmu_6->start_holdoff(0,FALSE);
	tmu_6->stop_holdoff(0,FALSE);
	wait.delay_10_us(10);
	...
	BFreq_Ton_S = Average_Value(tmeas_array, 1.5e-6, 3.5e-6);

	if(BFreq_Pst_S != 0)
	{
		period = 1 / BFreq_Pst_S;
		if(period != 0)
		{
			BFreq_DC_S = 100*BFreq_Ton_S / period;
		}
	}
	...
	PiDatalog(func, A_BFreq_Ton_S,     BFreq_Ton_S,          26, POWER_MICRO);
	PiDatalog(func, A_BFreq_DC_S,      BFreq_DC_S,           26, POWER_UNIT);
