...
void EEprom_DevID(test_function& func)
{
	...
	//DSM_init_LYT8();      //Moved to Leakage function due to repeatability failure
		PowerUp_I2C_P();
		...
		else
		{
			...
			if(pE0_data == 0 && pE2_data == 0 && pE4_data == 0 && pE6_data == 0&& pE8_data ==0&&Primary_Die_ID != 0)
			{
				g_Erase_Enable_P = 0;
				g_Sim_Enable_P	 = 1;
				Pri_Untrimmed	 = 1;
		...
		//---------------------------------------------------------------------------------
		if( g_OPCODE==4200 && (g_Erase_Enable_P||Blind_Trim))
		{
			if(g_Erase_Enable_P)
			{
				EEPROM_ERASE_P(&g_vDrain_Erase_th_P);
			...
			//if(Blind_Trim)
			//{
			//	printf("\t\t Enter Blind Trim.\n");
			//	//HL. This is needed or won't be able to burn EEprom.
			...
			//	if(g_DOPL_Primary)
			//	{
			//		DSM_I2C_Write('w', 0xE6, 0x0040); // DOPL bit set to 1 for REL}
			//	}
			//	else
			//	{
			//		DSM_I2C_Write('w', 0xE6, 0x0000);   // Not Trim DOPL bit
			//	}
			//	DSM_I2C_Write('w', 0xE8, 0x0000); 
		...
		//This is to read back for debugging only.
		if(1)
		{
			g_Debug=0;
			EEPROM_Read_Enable_P();
			int C0_Read = DSM_I2C_Read(0xC0);
			int C2_Read = DSM_I2C_Read(0xC2);
			int C4_Read = DSM_I2C_Read(0xC4);
			int C6_Read = DSM_I2C_Read(0xC6);
			int C8_Read = DSM_I2C_Read(0xC8);
			g_Debug=0;
...
///////////         7.  Re-Read EEprom 0xC0 to ensure if it's erased properly
	...
	//Not sure why some units reading Secondary ID = 0.  Unable to find the unit yet.
	i = 0;
	int	exit_loop = 0;
	...
	if(g_Erase_Enable_S&&g_Device_ID_S!=0)
	{
		...
		DSM_I2C_Write('w', g_EEP_CMD, 0x0003);
