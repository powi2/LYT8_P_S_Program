...
void VR_Det_01_S_user_init(test_function& func)
{
	VR_Det_01_S_params *ours;
    ours = (VR_Det_01_S_params *)func.params;
...
// Datalog gFuncNum variable //
	if(gDisplay_FuncNum)
		PiDatalog(func, A_Func_Num_VR_Det_01_S, gFuncNum, 27, POWER_UNIT);

	// Skip Test if AbortTest set 
	if (AbortTest)
		return;

	if(g_Trim_TvrOption_B16_17_S_Trimops != 1)
		return;
...
Pulse pulse;
	...
	////////g_DDD_High = 2.5; 
	////////
	////////ddd_7->ddd_set_clock_period(1e-6);			
	////////ddd_7->ddd_set_lo_level(0.01);
	////////ddd_7->ddd_set_hi_level(2.5);	
	...
	if(1) //internal HBP
	{
		HBP_dvi2k->set_current(HBP_ch, 0.000e-9, RANGE_200_MA);
		HBP_dvi2k->set_voltage(HBP_ch, 0.0, VOLT_20_RANGE);
		delay(2);
	}
	else //External HbP
	{
		HBP_dvi2k->set_current(HBP_ch, 10e-3, RANGE_200_MA);
	...
	//Ramp VR and observe output on Boost pin.
	B_ovi3->set_meas_mode(B_ch1, OVI_MEASURE_CURRENT);
	VR_dvi->set_current(VRstep_ch, 100e-3, RANGE_300_MA);
	j = 10.0;
	i = 0;
...
VR_Detect_A_Search(B1_FastUpdate, B2_MaxSate, A_Skipping, A1_MinState, A2_FastUpdate, A3_Target_VR, VStart, Vstop, VstepSize);
	...
	PiDatalog(func, A_VR1_Opt,		  g_Trim_TvrOption_B16_17_S_Trimops,      26, POWER_UNIT);
	PiDatalog(func, A_AVR1_Tap1,		  VFB_Meas[4],               26, POWER_UNIT);
	PiDatalog(func, A_AVR1_Tap2,		  VFB_Meas[1],               26, POWER_UNIT);
	PiDatalog(func, A_AVR1_Tap3,		  VFB_Meas[2],               26, POWER_UNIT);
	PiDatalog(func, A_AVR1_Tap4,		  VFB_Meas[3],               26, POWER_UNIT);

	PiDatalog(func, A_VFB_InHBP_44_01,		  VFB,               26, POWER_UNIT);
	...
	PiDatalog(func, A_A0_InHBP_44_01, A_Skipping,				 26, POWER_UNIT);
	PiDatalog(func, A_A1_InHBP_44_01, A1_MinState,				 26, POWER_UNIT);
	PiDatalog(func, A_A2_InHBP_44_01, A2_FastUpdate,			 26, POWER_UNIT);
	PiDatalog(func, A_A3_InHBP_44_01, A3_Target_VR,				 26, POWER_UNIT);


	PiDatalog(func, A_B1_InHBP_44_01,	  B1_FastUpdate,		 26, POWER_UNIT);
	PiDatalog(func, A_B2_InHBP_44_01,	  B2_MaxSate,			 26, POWER_UNIT);

	PiDatalog(func, A_BSk_InHBP_44_01,  B_Skip_5K,				 26, POWER_UNIT);
	PiDatalog(func, A_BD_InHBP_44_01,	  B_5K,				     26, POWER_UNIT);

	PiDatalog(func, A_BLOSkInHBP44_01,	 BLO_Skip_5K,				 26, POWER_UNIT);
	PiDatalog(func, A_BLO_InHBP44_01,	  BLO_5K,				     26, POWER_UNIT);
	PiDatalog(func, A_B3_InHBP_44_01,	  B3_UVLO_5K,				 26, POWER_UNIT);
